...
import { getMarkedMoviesSchema, markMovieSchema, removeMarkedMovieSchema, searchMovieByIdSchema, searchMoviesSchema } from "@anytrack/joi";

const movieRouter = Router();

movieRouter.get("/search", userMiddleware, joiMiddleware(searchMoviesSchema, "query"), getMovieByTerm);
movieRouter.get("/mark", userMiddleware, joiMiddleware(getMarkedMoviesSchema, 'query'), getMarkedMovies);
